...
import KakaoMapTest from '../sell-service/kakaoMapComponents/KakaoMapTest';
...
function BoardDetail() {
  ...
  const previewImage = `http://localhost:8000/sell-service/img/${sellId}`
  ...
  useEffect(() => {
    fetchFn("GET", `http://localhost:8000/board-service/boards/${sellId}`)
        .then(data => {
          setBoard(data)
        })
  ...
  return (
    <div>
       ...
       <img style={{ maxHeight: '200px' , maxWidth: '400px', display:'inline-block', marginLeft: '5%'}}
        ...
        {/* 거래정보 */}
       <div className='border' style={{ verticalAlign: '-13px',display: 'inline-block', width: '450px', height: '210px', paddingInline: '3%', marginLeft: '1%', fontSize: 'larger'}}>
        
         판매자 : {board.username} <button className='detailbutton' onClick={() => openPopup(`/chatwith/${board.username}`)}><img src='/img/chatting.png' style={{widthd: '30px', height: '30px'}}/></button><br/>
