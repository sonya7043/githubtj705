...
function KakaoMapSellList({ setRangedSellList }) {
  ...
  useEffect(() => {
    const initLongitude = localStorage.getItem("longitude");
    const initLatitude = localStorage.getItem("latitude");
  ...
  const sellListSearchButtonHandler = async () => {
    if (currentCircle) {
      ...
      const sellList = await fetch_General("GET", `http://localhost:8000/sell-service/sells/findbyrange?range=${inputRange.current.value}`)
      if (sellList) {
